...
/**
 * Download and apply the latest alphhaspace update from GitHub Releases
 *
...
export async function downloadAndApplyUpdate(
  ...
  try {
    ...
    // See: https://github.com/AndyMik90/Alphhaspace/issues/78
    ...
    const tarballPath = path.join(cachePath, 'alphhaspace-update.tar.gz');
    ...
    // Find the alphhaspace folder in extracted content
    ...
    if (!existsSync(autoBuildSource)) {
      throw new Error('alphhaspace folder not found in download');
    }
