...
export function registerTaskExecutionHandlers(
    ...
    (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      if (!gitStatus.isGitRepo) {
        ...
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          'Git repository required. Please run "git init" in your project directory. Alphhaspace uses git worktrees for isolated builds.'
        );
        return;
      }
    ...
    async (
      ...
      if (approved) {
        ...
        if (hasWorktree) {
          ...
          // IMPORTANT: Exclude .alphhaspace and .worktrees directories to preserve specs and worktree data
          const cleanResult = spawnSync('git', ['clean', '-fd', '-e', '.alphhaspace', '-e', '.worktrees'], {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: 'pipe'
          });
          if (cleanResult.status === 0) {
            console.log('[TASK_REVIEW] Cleaned untracked files in main (excluding .alphhaspace and .worktrees)');
          }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // Get the spec directory
      const autoBuildDir = project.autoBuildPath || '.alphhaspace';
      const specDir = path.join(
        project.path,
        autoBuildDir,
        'specs',
