...
import { getEffectiveSourcePath } from '../../alphhaspace-updater';
      ...
      try {
        ...
        if (!pythonEnvManager.isEnvReady()) {
          ...
          if (autoBuildSource) {
            ...
            return { success: false, error: 'Python environment not ready and Alphhaspace source not found' };
          }
        }
        ...
        if (!sourcePath) {
          return { success: false, error: 'Alphhaspace source not found' };
        }

        const runScript = path.join(sourcePath, 'run.py');
        const specDir = path.join(project.path, project.autoBuildPath || '.alphhaspace', 'specs', task.specId);
              ...
              if (isStageOnly) {
                try {
                  ...
                  if (!hasActualStagedChanges) {
                    ...
                    const specBranch = `alphhaspace/${task.specId}`;
                    try {
      ...
      try {
        ...
        if (!pythonEnvManager.isEnvReady()) {
          ...
          if (autoBuildSource) {
            ...
            console.error('[IPC] Alphhaspace source not found');
            return { success: false, error: 'Python environment not ready and Alphhaspace source not found' };
          }
        }
