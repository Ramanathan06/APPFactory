...
import { checkForUpdates as checkSourceUpdates, downloadAndApplyUpdate, getBundledVersion, getEffectiveVersion, getEffectiveSourcePath } from '../alphhaspace-updater';
...
export function registerAutobuildSourceHandlers(
  ...
  // ============================================
  // Alphhaspace Source Update Operations
  // ============================================
  ...
  // ============================================
  // Alphhaspace Source Environment Operations
  // ============================================
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        if (!sourcePath) {
          return {
            success: false,
            error: 'Alphhaspace source path not found. Please configure it in App Settings.'
          };
        }
        ...
        if (!sourcePath) {
          return {
            ...
            data: {
              hasToken: false,
              sourcePath: undefined,
              error: 'Alphhaspace source path not found'
            }
          };
        }
