 ...
 */
const GITIGNORE_ENTRIES = ['.alphhaspace/'];
...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (existsSync(gitignorePath)) {
    ...
    writeFileSync(gitignorePath, '# Alphhaspace data directory\n' + entriesToAdd.join('\n') + '\n');
  }
...
/**
 * Data directories created in .alphhaspace for each project
 */
...
/**
 * Check if project is initialized (has .alphhaspace directory)
 */
export function isInitialized(projectPath: string): boolean {
  const dotAutoBuildPath = path.join(projectPath, '.alphhaspace');
  return existsSync(dotAutoBuildPath);
}

/**
 * Initialize alphhaspace data directory in a project.
 *
 * Creates .alphhaspace/ with data directories (specs, ideation, insights, roadmap).
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  // Check git status - Alphhaspace requires git for worktree-based builds
  const gitStatus = checkGitStatus(projectPath);
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    debug('Git check failed', { gitStatus });
    return {
      success: false,
      error: gitStatus.error || 'Git repository required. Alphhaspace uses git worktrees for isolated builds.'
    };
  }
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Already initialized - .alphhaspace exists');
    return {
      success: false,
      error: 'Project already has alphhaspace initialized (.alphhaspace exists)'
    };
  }

  try {
    debug('Creating .alphhaspace data directory', { dotAutoBuildPath });

    // Create the .alphhaspace directory
    ...
    // Update .gitignore to exclude .alphhaspace/
...
/**
 * Ensure all data directories exist in .alphhaspace.
...
/**
 * Get the alphhaspace folder path for a project.
 *
 * IMPORTANT: Only .alphhaspace/ is considered a valid "installed" alphhaspace.
 * The alphhaspace/ folder (if it exists) is the SOURCE CODE being developed,
 * not an installation. This allows Alphhaspace to be used to develop itself.
 */
export function getAutoBuildPath(projectPath: string): string | null {
  ...
  if (existsSync(dotAutoBuildPath)) {
    debug('Returning .alphhaspace (installed version)');
    return '.alphhaspace';
  }

  debug('No .alphhaspace folder found - project not initialized');
  return null;
}
