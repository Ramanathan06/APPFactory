...
export class ProjectStore {
  ...
  addProject(projectPath: string, name?: string): Project {
    ...
    if (existing) {
      // Validate that .alphhaspace folder still exists for existing project
      ...
      if (existing.autoBuildPath && !isInitialized(existing.path)) {
        console.warn(`[ProjectStore] .alphhaspace folder was deleted for project "${existing.name}" - resetting autoBuildPath`);
    ...
    // Determine alphhaspace path (supports both 'alphhaspace' and '.alphhaspace')
  ...
  /**
   * Validate all projects to ensure their .alphhaspace folders still exist.
   ...
   *
   * @returns Array of project IDs that were reset due to missing .alphhaspace folder
   */
  validateProjects(): string[] {
    ...
    for (const project of this.data.projects) {
      ...
      // Check if .alphhaspace folder still exists
      if (!isInitialized(project.path)) {
        console.warn(`[ProjectStore] .alphhaspace folder missing for project "${project.name}" at ${project.path}`);
    ...
    if (hasChanges) {
      this.save();
      console.warn(`[ProjectStore] Reset ${resetProjectIds.length} project(s) due to missing .alphhaspace folder`);
    }
