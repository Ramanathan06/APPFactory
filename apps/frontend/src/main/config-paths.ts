 ...
 * This is where graph databases are stored (previously ~/.alphhaspace/memories)
 */
export function getMemoriesDir(): string {
  // For compatibility, we still support the legacy path
  const legacyPath = path.join(os.homedir(), '.alphhaspace', 'memories');
