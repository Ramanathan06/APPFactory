...
export class AgentQueueManager {
  ...
  private spawnIdeationProcess(
    ...
    // Run from alphhaspace source directory so imports work correctly
    const autoBuildSource = this.processManager.getAutoBuildSourcePath();
    ...
    // 3. combinedEnv (alphhaspace/.env for CLI usage)
    ...
    const tokenSource = profileEnv['CLAUDE_CODE_OAUTH_TOKEN']
      ...
      : (combinedEnv['CLAUDE_CODE_OAUTH_TOKEN'] ? 'alphhaspace/.env' : 'not found');
