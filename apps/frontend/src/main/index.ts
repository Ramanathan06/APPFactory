...
app.setName('Alphhaspace');
if (process.platform === 'darwin') {
  // Force the name to appear in dock on macOS
  app.name = 'Alphhaspace';
}
...
app.whenReady().then(() => {
  ...
  // Load settings and configure agent manager with Python and alphhaspace paths
  ...
  try {
    ...
    if (validAutoBuildPath) {
      ...
      if (!specRunnerExists) {
        ...
        // Old structure: /path/to/project/alphhaspace
        // New structure: /path/to/project/apps/backend
        let migrated = false;
        if (validAutoBuildPath.endsWith('/alphhaspace') || validAutoBuildPath.endsWith('\\alphhaspace')) {
          const basePath = validAutoBuildPath.replace(/[/\\]alphhaspace$/, '');
