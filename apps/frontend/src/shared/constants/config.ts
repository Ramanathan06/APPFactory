...
// IMPORTANT: All paths use .alphhaspace/ (the installed instance), NOT alphhaspace/ (source code)
export const AUTO_BUILD_PATHS = {
  SPECS_DIR: '.alphhaspace/specs',
  ROADMAP_DIR: '.alphhaspace/roadmap',
  IDEATION_DIR: '.alphhaspace/ideation',
  ...
  PROJECT_INDEX: '.alphhaspace/project_index.json',
  GRAPHITI_STATE: '.graphiti_state.json'
} as const;

/**
 * Get the specs directory path.
 * All specs go to .alphhaspace/specs/ (the project's data directory).
 */
export function getSpecsDir(autoBuildPath: string | undefined): string {
  const basePath = autoBuildPath || '.alphhaspace';
