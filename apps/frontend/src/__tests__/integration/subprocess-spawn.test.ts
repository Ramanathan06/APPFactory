...
const AUTO_CLAUDE_SOURCE = path.join(TEST_DIR, 'alphhaspace-source');

// Setup test directories
function setupTestDirs(): void {
  mkdirSync(TEST_PROJECT_PATH, { recursive: true });

  // Create alphhaspace source directory that getAutoBuildSourcePath looks for
    ...
    it('should spawn Python process for spec creation', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: AUTO_CLAUDE_SOURCE,  // Process runs from alphhaspace source directory
