...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/alphhaspace/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .alphhaspace or .alphhaspace-status."""
    cwd = Path.cwd()

    # Check current directory - prioritize .alphhaspace (installed instance)
    if (cwd / ".alphhaspace").exists():
        return cwd
    if (cwd / ".alphhaspace-status").exists():
        return cwd
    ...
    for parent in cwd.parents:
        if (parent / ".alphhaspace").exists():
            return parent
        if (parent / ".alphhaspace-status").exists():
            return parent
