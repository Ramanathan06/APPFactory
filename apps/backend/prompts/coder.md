...
- **Spec Location**: Where your spec files live (usually `./alphhaspace/specs/{spec-name}/`)
...
SPEC_DIR="./alphhaspace/specs/YOUR-SPEC-NAME"  # Replace with actual path from step 2
...
```bash
git add .
git commit -m "alphhaspace: Complete [subtask-id] - [subtask description]
...
**Note:** The `build-progress.txt` file is in `.alphhaspace/specs/` which is gitignored.
...
Total phases: [N]
Total subtasks: [N]
Branch: alphhaspace/[feature-name]
