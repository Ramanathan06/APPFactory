...
Usage:
    python -m alphhaspace.merge.tracker_cli notify-commit <hash>
    python -m alphhaspace.merge.tracker_cli show-timeline <file_path>
    python -m alphhaspace.merge.tracker_cli show-drift <task_id>
"""
...
def find_project_root() -> Path:
    """Find the project root by looking for .alphhaspace or .git directory."""
    current = Path.cwd()

    # Walk up until we find .alphhaspace or .git
    while current != current.parent:
        if (current / ".alphhaspace").exists() or (current / ".git").exists():
            return current
