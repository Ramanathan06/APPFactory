...
Usage:
    python -m alphhaspace.merge.install_hook [--project-path /path/to/project]
"""
...
# Skip if we're in a worktree (alphhaspace branches)
if [[ "$BRANCH" == "main" ]] || [[ "$BRANCH" == "master" ]]; then
    ...
    if [[ -d ".git" ]]; then
        ...
        ($PYTHON -m alphhaspace.merge.tracker_cli notify-commit "$COMMIT_HASH" 2>/dev/null &) &
