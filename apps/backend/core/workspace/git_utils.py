...
def get_changed_files_from_branch(
    ...
    Args:
        ...
        exclude_alphhaspace: If True, exclude .alphhaspace directory files (default True)
    ...
    if result.returncode == 0:
        for line in result.stdout.strip().split("\n"):
            if line:
                ...
                if len(parts) == 2:
                    file_path = parts[1]
                    # Exclude .alphhaspace directory files from merge
                    if exclude_alphhaspace and _is_alphhaspace_file(file_path):
                        continue
                    files.append((file_path, parts[0]))  # (file_path, status)
    return files


def _is_alphhaspace_file(file_path: str) -> bool:
    """Check if a file is in the .alphhaspace or alphhaspace/specs directory."""
