...
def merge_existing_build(
    ...
    Called when user runs: python alphhaspace/run.py --spec X --merge
    ...
    if not worktree_path:
        ...
        print(highlight(f"  python alphhaspace/run.py --spec {spec_name}"))
        return False
    ...
    spec_branch = f"alphhaspace/{spec_name}"
    ...
    if current_branch == spec_branch:
        ...
        print(highlight(f"  python alphhaspace/run.py --spec {spec_name} --merge"))
        return False
        ...
        if smart_result is not None:
            ...
            if smart_result.get("success"):
                ...
                if had_conflicts or files_merged or ai_assisted:
                    ...
                    # User can delete with: python alphhaspace/run.py --spec <name> --discard
    ...
    if success_result:
        ...
        if no_commit:
            ...
            print(muted(f"  python alphhaspace/run.py --spec {spec_name} --discard"))
        else:
    ...
    try:
        ...
        if merge_tree_result.returncode != 0:
            ...
            for line in output.split("\n"):
                if "CONFLICT" in line:
                    ...
                    if match:
                        ...
                        # Skip .alphhaspace files - they should never be merged
                        if (
                            file_path
                            and file_path not in result["conflicting_files"]
                            and not _is_alphhaspace_file(file_path)
                        ):
            ...
            if not result["conflicting_files"]:
                ...
                # Filter out .alphhaspace files - they should never be merged
                conflicting = main_files & spec_files
                result["conflicting_files"] = [
                    f for f in conflicting if not _is_alphhaspace_file(f)
                ]
...
def _resolve_git_conflicts_with_ai(
    ...
    spec_branch = git_conflicts.get("spec_branch", f"alphhaspace/{spec_name}")
