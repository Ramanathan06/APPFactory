...
- Branch name: alphhaspace/{spec-name}
...
class WorktreeManager:
    ...
    a corresponding branch alphhaspace/{spec-name}.
    """
    ...
    def _unstage_gitignored_files(self) -> None:
        ...
        plus any files in the .alphhaspace directory which should never be merged.

        This is needed after a --no-commit merge because files that exist in the
        source branch (like spec files in .alphhaspace/specs/) get staged even if
        ...
        # Files to unstage: gitignored files + .alphhaspace directory files
        ...
        # 2. Always unstage .alphhaspace directory files - these are project-specific
        # and should never be merged from the worktree branch
        alphhaspace_patterns = [".alphhaspace/", "alphhaspace/specs/"]
        for file in staged_files:
            file = file.strip()
            if not file:
                continue
            for pattern in alphhaspace_patterns:
        ...
        if files_to_unstage:
            print(
                f"Unstaging {len(files_to_unstage)} alphhaspace/gitignored file(s)..."
            )
            # Unstage each file
    ...
    def get_branch_name(self, spec_name: str) -> str:
        ...
        return f"alphhaspace/{spec_name}"
    ...
    def _check_branch_namespace_conflict(self) -> str | None:
        """
        Check if a branch named 'alphhaspace' exists, which would block creating
        branches in the 'alphhaspace/*' namespace.

        Git stores branch refs as files under .git/refs/heads/, so a branch named
        'alphhaspace' creates a file that prevents creating the 'alphhaspace/'
        directory needed for 'alphhaspace/{spec-name}' branches.

        Returns:
            The conflicting branch name if found, None otherwise.
        """
        result = self._run_git(["rev-parse", "--verify", "alphhaspace"])
        if result.returncode == 0:
            return "alphhaspace"
        return None
    ...
    def create_worktree(self, spec_name: str) -> WorktreeInfo:
        ...
        # Check for branch namespace conflict (e.g., 'alphhaspace' blocking 'alphhaspace/*')
        ...
        if conflicting_branch:
            raise WorktreeError(
                ...
                f"\n"
                f"Git branch names work like file paths - a branch named 'alphhaspace' prevents\n"
                f"creating branches under 'alphhaspace/' (like 'alphhaspace/{spec_name}').\n"
                f"\n"
    ...
    def merge_worktree(
        ...
        if no_commit:
            ...
            merge_args.extend(["-m", f"alphhaspace: Merge {info.branch}"])
    ...
    def list_all_spec_branches(self) -> list[str]:
        """List all alphhaspace branches (even if worktree removed)."""
        result = self._run_git(["branch", "--list", "alphhaspace/*"])
        if result.returncode != 0:
            return []
...
STAGING_WORKTREE_NAME = "alphhaspace"
