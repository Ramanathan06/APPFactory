...
def load_project_mcp_config(project_dir: Path) -> dict:
    """
    Load MCP configuration from project's .alphhaspace/.env file.
    ...
    """
    env_path = project_dir / ".alphhaspace" / ".env"
...
def create_client(
    ...
    # Check if custom alphhaspace tools are available
    alphhaspace_tools_enabled = is_tools_available()
    ...
    # Load per-project MCP configuration from .alphhaspace/.env
    ...
    if "alphhaspace" in required_servers and alphhaspace_tools_enabled:
        mcp_servers_list.append(f"alphhaspace ({agent_type} tools)")
    if mcp_servers_list:
    ...
    # Add custom alphhaspace MCP server if required and available
    if "alphhaspace" in required_servers and alphhaspace_tools_enabled:
        alphhaspace_mcp_server = create_alphhaspace_mcp_server(spec_dir, project_dir)
        if alphhaspace_mcp_server:
            mcp_servers["alphhaspace"] = alphhaspace_mcp_server
