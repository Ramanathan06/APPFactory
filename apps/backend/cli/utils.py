...
Shared utility functions for the Alphhaspace CLI.
"""
...
def setup_environment() -> Path:
    ...
    # Load .env file - check both alphhaspace/ and dev/alphhaspace/ locations
    env_file = script_dir / ".env"
    dev_env_file = script_dir.parent / "dev" / "alphhaspace" / ".env"
    if env_file.exists():
...
def find_spec(project_dir: Path, spec_identifier: str) -> Path | None:
    ...
    if worktree_base.exists():
        ...
        worktree_spec = (
            worktree_base / spec_identifier / ".alphhaspace" / "specs" / spec_identifier
        )
        ...
        for worktree_dir in worktree_base.iterdir():
            if worktree_dir.is_dir() and worktree_dir.name.startswith(
                ...
                spec_in_worktree = (
                    worktree_dir / ".alphhaspace" / "specs" / worktree_dir.name
                )
                if (
...
def validate_environment(spec_dir: Path) -> bool:
    ...
    if not get_auth_token():
        ...
        print("\nAlphhaspace requires Claude Code OAuth authentication.")
    ...
    if is_linear_enabled():
        ...
        )  # alphhaspace/specs/001-name -> project root
