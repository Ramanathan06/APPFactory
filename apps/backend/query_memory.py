#!/usr/bin/env python3
"""
Memory Query CLI for alphhaspace-ui.
...
async def _async_semantic_search(args):
    ...
    try:
        # Add alphhaspace to path for imports
        alphhaspace_dir = Path(__file__).parent
        if str(alphhaspace_dir) not in sys.path:
...
def main():
    parser = argparse.ArgumentParser(
        description="Query LadybugDB memory database for alphhaspace-ui"
    )
