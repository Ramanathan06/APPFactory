...
Usage:
    python alphhaspace/spec_runner.py --task "Add user authentication"
    python alphhaspace/spec_runner.py --interactive
    python alphhaspace/spec_runner.py --continue 001-feature
    python alphhaspace/spec_runner.py --task "Fix button color" --complexity simple
    python alphhaspace/spec_runner.py --task "Simple fix" --no-ai-assessment
"""
...
if sys.version_info < (3, 10):  # noqa: UP036
    sys.exit(
        f"Error: Alphhaspace requires Python 3.10 or higher.\n"
...
# Add alphhaspace to path (parent of runners/)
...
dev_env_file = Path(__file__).parent.parent.parent / "dev" / "alphhaspace" / ".env"
if env_file.exists():
...
Examples:
  ...
  python spec_runner.py --interactive
    ...
    # Find project root (look for alphhaspace folder)
    project_dir = args.project_dir

    # Auto-detect if running from within alphhaspace directory (the source code)
    if project_dir.name == "alphhaspace" and (project_dir / "run.py").exists():
        # Running from within alphhaspace/ source directory, go up 1 level
        project_dir = project_dir.parent
    elif not (project_dir / ".alphhaspace").exists():
        # No .alphhaspace folder found - try to find project root
        # First check for .alphhaspace (installed instance)
        for parent in project_dir.parents:
            if (parent / ".alphhaspace").exists():
                project_dir = parent
                break
    ...
    try:
        ...
        if not args.no_build:
            ...
            if not review_state.is_approved():
                ...
                print(
                    f"  {highlight(f'python alphhaspace/review.py --spec-dir {orchestrator.spec_dir}')}"
                )
                print()
                print(
                ...
                example_cmd = (
                    'python alphhaspace/spec_runner.py --task "..." --auto-approve'
                )
        ...
        print(
            f"To continue: python alphhaspace/spec_runner.py --continue {orchestrator.spec_dir.name}"
        )
        sys.exit(1)
