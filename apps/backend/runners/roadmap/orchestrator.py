...
class RoadmapOrchestrator:
    ...
    def __init__(
        ...
        # Default output to project's .alphhaspace directory (installed instance)
        # Note: alphhaspace/ is source code, .alphhaspace/ is the installed instance
        if output_dir:
            self.output_dir = Path(output_dir)
        else:
            # Initialize .alphhaspace directory and ensure it's in .gitignore
            init_alphhaspace_dir(self.project_dir)
            self.output_dir = self.project_dir / ".alphhaspace" / "roadmap"
