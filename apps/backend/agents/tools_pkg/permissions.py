...
def get_allowed_tools(
    ...
    # Add alphhaspace tools ONLY if the MCP server is available
    # This prevents allowing tools that won't work because the server isn't running
    if "alphhaspace" in required_servers and is_tools_available():
        tools.extend(config.get("alphhaspace_tools", []))
...
def _get_mcp_tools_for_servers(servers: list[str]) -> list[str]:
    ...
    for server in servers:
        ...
        # alphhaspace tools are already added via config["alphhaspace_tools"]
