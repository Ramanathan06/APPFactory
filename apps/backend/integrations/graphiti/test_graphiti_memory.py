...
alphhaspace_dir = Path(__file__).parent.parent.parent
sys.path.insert(0, str(alphhaspace_dir))

# Load .env file
try:
    from dotenv import load_dotenv

    env_file = alphhaspace_dir / ".env"
    if env_file.exists():
...
async def test_ollama_embeddings() -> bool:
    ...
    try:
        ...
        test_text = (
            "This is a test embedding for Alphhaspace memory system using LadybugDB."
        )
...
async def main():
    ...
    parser.add_argument(
        "--db-path",
        default=os.path.expanduser("~/.alphhaspace/memories"),
        help="Database path",
    )
    parser.add_argument(
        "--database",
    ...
    print(
        "    python integrations/graphiti/test_graphiti_memory.py --database alphhaspace_memory"
    )
    print()
