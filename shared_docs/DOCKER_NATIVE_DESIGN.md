...
> Design document for converting Alphhaspace from Electron to a containerized web application.

## Executive Summary

This document outlines the architecture for a Docker-native version of Alphhaspace that:
...
| POST | `/api/projects/{id}/initialize` | Initialize alphhaspace in project |
| GET | `/api/projects/{id}/version` | Check alphhaspace version |
...
WORKDIR /app
COPY alphhaspace/requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Install API dependencies
COPY alphhaspace/api/requirements.txt ./api-requirements.txt
RUN pip install --no-cache-dir -r api-requirements.txt

# Copy application code
COPY alphhaspace/ ./alphhaspace/

# --- Frontend Build Stage ---
FROM node:22-alpine AS frontend-build

WORKDIR /app
COPY alphhaspace-ui/package*.json ./
RUN npm ci
...
# Environment
ENV PYTHONPATH=/app/alphhaspace
ENV DATA_DIR=/data
...
services:
  app:
    ...
    volumes:
      ...
      # Persistent data
      - alphhaspace-data:/data
      # Claude CLI config (for OAuth tokens)
      - alphhaspace-claude:/home/claude/.claude
    environment:
  ...
  falkordb:
    ...
    container_name: alphhaspace-falkordb
    volumes:
      - falkordb-data:/data
    networks:
      - alphhaspace-net
  ...
  graphiti-mcp:
    ...
    container_name: alphhaspace-graphiti
...
1. OAuth tokens stored in named volume (`alphhaspace-claude`)
