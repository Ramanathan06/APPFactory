...
# Add alphhaspace directory to path for imports
...
mock_init = MagicMock()
mock_init.init_alphhaspace_dir = MagicMock(return_value=(Path("/tmp"), False))
...
class TestGetSpecsDir:
    ...
    def test_returns_specs_path(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.init_alphhaspace_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".alphhaspace", False)

            result = get_specs_dir(temp_dir)

            assert result == temp_dir / ".alphhaspace" / "specs"

    def test_calls_init_alphhaspace_dir(self, temp_dir: Path):
        """Initializes alphhaspace directory."""
        with patch('spec.pipeline.models.init_alphhaspace_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".alphhaspace", False)
...
class TestSpecOrchestratorInit:
    ...
    def test_init_with_project_dir(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.init_alphhaspace_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".alphhaspace", False)
            specs_dir = temp_dir / ".alphhaspace" / "specs"
