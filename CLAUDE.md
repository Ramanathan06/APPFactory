...
Alphhaspace is a multi-agent autonomous coding framework that builds software through coordinated AI agent sessions. It uses the Claude Agent SDK to run agents in isolated workspaces with security controls.
...
Alphhaspace uses git worktrees for isolated builds. All branches stay LOCAL until user explicitly pushes:
...
**Key principles:**
- ONE branch per spec (`alphhaspace/{spec-name}`)
...
**CRITICAL: When submitting PRs to AndyMik90/Alphhaspace, always target the `develop` branch, NOT `main`.**
...
5. Create PR targeting `develop`: `gh pr create --repo AndyMik90/Alphhaspace --base develop`
...
**CRITICAL: Alphhaspace uses the Claude Agent SDK for ALL AI interactions. Never use the Anthropic API directly.**
...
Alphhaspace uses Graphiti as its primary memory system with embedded LadybugDB (no Docker required):
...
- Memory data stored in `.alphhaspace/specs/XXX/graphiti/`
...
- `.alphhaspace/specs/` - Per-project data (specs, plans, QA reports, memory) - gitignored
